...
var regexes = require('./parser.js').regexes;
var Unfold = require('./parser.js').unfold;
var ParseMail = require('./parser.js').parseMail;
...
ParseMail(mail);
console.log(ParseMail(mail).To);
console.log(ParseMail(mail).From);
console.log(ParseMail(mail).Date);

describe ('should have a function', function(){
        it('should have a parseMail()', function(){
          assert.isFunction(ParseMail);
        });
        it('should have a unfold()', function(){
          assert.isFunction(Unfold);
        });
});

describe ('should print out the request information', function(){
        it('Should be To Nicholas Hare', function(){
           assert.equal(ParseMail(mail).To, ' Nicholas Hare <oharemarketing@gmail.com>');
        });
        it('Should be From Michael Miller', function(){
           assert.equal(ParseMail(mail).From, ' Michael Miller <michael.miller@cloudspace.com>');
        });
        it('Should be dated Feb 25th', function(){
           assert.equal(ParseMail(mail).Date, ' Wed, 25 Feb 2015 14:42:09 -0500');
        });
        it('Should have the subject Coding Practicle', function(){
           assert.equal(ParseMail(mail).Subject, ' Coding Practicle');
        });
});
